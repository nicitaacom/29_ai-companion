...
export function Companions({ data, messages }: CompanionsProps) {
  ...
  return (
    <div className="grid grid-cols-2 gap-3 pb-10 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5" data-test="companions">
      {data.map((item, index) => {
        ...
        return (
          <article
            className="group overflow-hidden rounded-[22px] border border-white/5 bg-zinc-800/90
                       shadow-[0_16px_40px_-32px_rgba(0,0,0,0.9)] transition duration-300 hover:-translate-y-1
                       hover:border-white/10 hover:bg-zinc-800"
            key={item.id}>
            <Link href={`/chat/${item.id}`} className="flex h-full flex-col p-2.5">
              <div className="relative aspect-[4/4.35] overflow-hidden rounded-[16px] bg-zinc-700">
                ...
                <div
                  className="absolute left-2.5 top-2.5 rounded-full border border-white/10 bg-black/35 px-2 py-1
                             text-[10px] font-medium uppercase tracking-[0.18em] text-white/90 backdrop-blur-sm">
                  AI companion
                </div>
              </div>
              <div className="flex flex-1 flex-col px-1 pb-1 pt-3">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1.5 line-clamp-2 min-h-[2.5rem] text-sm leading-5 text-zinc-400">
                      {item.description}
                    </p>
