...
export function UserAuthenticatedContent({ user }: { user: User }) {
  ...
  return (
    <AuthModalShell
      ...
      description="You are signed in with Supabase. Your session, providers and future billing access all flow
                   through this account."
      asideTitle="Everything is connected."
      asideDescription="Your companion profile now uses the Supabase session directly, so Google, GitHub and
                        credentials all land in one place.">
      <div className="flex h-full flex-col justify-between">
        <div className="space-y-6">
          <div
            className="rounded-[24px] border border-white/10 bg-white/[0.05] p-5 shadow-[0_10px_30px_rgba(0,0,0,0.22)]
                       backdrop-blur-md">
            <div className="flex items-center gap-3">
              <div
                className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/[0.08] text-white ring-1
                           ring-white/10">
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {providers.map(provider => (
                <span
                  key={provider}
                  className="rounded-full border border-white/10 bg-white/[0.06] px-3 py-1 text-xs font-medium uppercase
                             tracking-[0.18em] text-white/[0.78]">
                  {provider}
                </span>
              ))}
            </div>
