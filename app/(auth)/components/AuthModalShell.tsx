...
export function AuthModalShell({
  ...
  return (
    <div
      className="isolate grid h-full w-full overflow-hidden rounded-[28px] border border-white/10 bg-[#111317]
                 text-zinc-100 shadow-[0_30px_100px_rgba(0,0,0,0.7)] lg:grid-cols-[0.92fr_1.08fr]">
      <OrganicCanvasBackground
        ...
        <div
          // eslint-disable-next-line max-len -- unbreakable Tailwind arbitrary-value gradient token
          className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(255,255,255,0.08),transparent_34%),radial-gradient(circle_at_bottom_right,rgba(255,255,255,0.05),transparent_34%)]"
        />
        <div className="relative flex items-center gap-3 text-xs font-semibold uppercase tracking-[0.28em]
                         text-white/70">
        ...
        <div
          className="relative flex items-center justify-between rounded-[22px] border border-white/10 bg-white/[0.06]
                     px-5 py-4 text-sm text-white/[0.72] backdrop-blur-md">
      ...
      <div
        // eslint-disable-next-line max-len -- unbreakable Tailwind arbitrary-value gradient token
        className="flex h-full min-h-0 flex-col overflow-y-auto bg-[linear-gradient(180deg,rgba(17,19,23,0.96),rgba(13,16,20,0.96))] p-6 sm:p-8 md:p-9 lg:p-10">
