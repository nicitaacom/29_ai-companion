import { addTarEntry, finalizeTar, parseTar, gzipBufferClient, gunzipBufferClient } from "./tarClient"
import { BACKUP_TABLES, getPublicUrl, type IBackupFileRef, type IBackupTableConfig } from "./backupConfig"
 ...
 * body's own `type`, so callers must pass a body whose `type` is already set correctly.
 */
...
export async function exportTables(
  ...
  const archive = new Blob([gz], { type: "application/gzip" })

  return { fileName, archive }
}

export interface ITablesImportResult {
...
async function readCsvEntries(file: File): Promise<Record<string, string>> {
  ...
  try {
    ...
    throw new Error(`${file.name} is not a valid .tar.gz archive: ${getErrorMessage(error)}`)
  }
 ...
 */
function coerceRowsForImport(config: IBackupTableConfig, rows: Record<string, string | null>[]): Record<string, unknown>[] {
    ...
    for (const [column, value] of Object.entries(row)) {
      ...
      if (numericColumns.has(column)) {
        ...
        try {
          ...
          throw new Error(
            `${config.name}.csv row ${rowIndex + 1}, column "${column}": not valid JSON — ${getErrorMessage(error)}`,
          )
        }
      }
    }
...
export async function importTables(
  ...
  const results: ITablesImportResult = { tables: [] }
  let done = 0
  ...
  for (const table of tablesToImport) {
    ...
    try {
      ...
      throw new Error(`Failed to parse ${table.name}.csv: ${getErrorMessage(error)}`)
    }
    ...
    for (let start = 0; start < rows.length; start += ROW_BATCH_SIZE) {
      ...
      const response = await fetch("/api/backup/rows", {
        method: "POST",
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to import ${table.name} (${response.status})${message ? `: ${message}` : ""}`)
      }
      const { rows: rowsUpserted, skipped } = await response.json()
...
/**
 * Export every stored file across the backup buckets as one .tar.gz, entirely in the browser:
 * fetch the file list from the server, download each file directly from Supabase's public CDN, and
 * pack them locally. The app server never touches Storage bytes, so there is no 60s function
 * timeout regardless of library size. Kept separate from table export so a files backup never has
 * to fetch or pack table rows.
 */
export async function exportFiles(
  ...
  const { files }: { files: IBackupFileRef[] } = await filesRes.json()
  ...
  const packed: Array<{ file: IBackupFileRef; buf: Buffer | null }> = new Array(files.length)
  let nextIndex = 0
  ...
  const gz = await gzipBufferClient(new Uint8Array(tarBuf))
  const date = new Date().toISOString().slice(0, 10)
  const fileName = `backup-files-${date}.tar.gz`
  const archive = new Blob([gz], { type: "application/gzip" })
...
type ArchiveFile = { bucket: string; path: string; bytes: Uint8Array<ArrayBuffer>; contentType: string }
type UploadTarget =
...
export async function importFiles(
  ...
  if (archiveFiles.length === 0) {
    throw new Error("No storage files found in the archive — expected storage/<bucket>/… entries (e.g. storage/food-live/…).")
  }
  ...
  function bumpStat(bucket: string, key: "files" | "failed") {
  ...
  for (let start = 0; start < archiveFiles.length; start += URL_BATCH_SIZE) {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to prepare file upload (${response.status})${message ? `: ${message}` : ""}`)
    }

    const { results }: { results: UploadTarget[] } = await response.json()
    ...
    for (const target of results) {
      ...
      try {
        ...
        bumpStat(target.bucket, "failed")
        if (!firstErrorByBucket[target.bucket]) firstErrorByBucket[target.bucket] = getErrorMessage(error)
      }
    }
  }
  ...
  if (totalUploaded === 0) {
    ...
    throw new Error(
      firstError ?? "No files were uploaded — every path was skipped (import tables first so the files are recognized as yours).",
    )
  }
