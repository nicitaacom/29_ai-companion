...
function computeChecksum(header: Buffer): number {
  let sum = 0
  for (let headerIndex = 0; headerIndex < 512; headerIndex++) sum += header[headerIndex]
  return sum
}
...
export function addTarEntry(chunks: Buffer[], name: string, data: Buffer): void {
  if (name.length > 100) {
    buildLongNameEntry(name).forEach(chunk => chunks.push(chunk))
  }
 ...
 */
export async function gzipBufferClient(input: Uint8Array<ArrayBuffer>): Promise<Uint8Array<ArrayBuffer>> {
 ...
 */
export async function gunzipBufferClient(input: Uint8Array<ArrayBuffer>): Promise<Uint8Array<ArrayBuffer>> {
