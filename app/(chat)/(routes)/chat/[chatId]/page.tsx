...
export default async function ChatIdPage({ params }: ChatIdPageProps) {
  ...
  const { data: companion } = await supabase.from("companion").select("*").eq("id", params.chatId).single()

  // Fetch messages for the companion
  const { data: messages, error } = await supabase.from("messages").select("*").eq("id", params.chatId).order("created_at", { ascending: true })
