...
**Status**: every rule in the table below is wired into `eslint.config.mjs` and `pnpm lint` currently
passes with zero errors and zero warnings. `no-banned-words` renamed the companion `instructions`
field to `prompt` in code - the actual Supabase column still needs `ALTER TABLE "29_companion" RENAME
COLUMN instructions TO prompt` run against the database (not run yet) before this is fully live; see
the `update-types-do-not-run` script name in `package.json` as the reminder not to regenerate
`types_db.ts` until that migration has actually happened.
...
1. **Copy the rules folder as-is.**
   ...
   ```bash
   cp -r eslint-local-rules/ /path/to/other-project/eslint-local-rules/
   ```
...
## SOP: adding a new custom local rule

1. Create the rule file `eslint-local-rules/your-rule-name.js`  
   Follow the structure of simple existing rules (e.g. `no-handlers-variable.js` or `no-inner-component.js`).  
   Must export exactly like this:
   ```js
   module.exports = {
     "your-rule-name": {
       meta: { type: "problem", docs: { description: "..." }, messages: { ... } },
       create(context) { return { ... } }
     }
   }
   ```

2. Wire it in `eslint-local-rules/index.js` (no comments on the line):
   ```js
   ...require("./your-rule-name"),
   ```

3. Enable it in `eslint.config.mjs` (in the main rules object):
   ```js
   "local-rules/your-rule-name": "warn",
   ```

4. Document in this file (`dev_readme-eslint.md`):
   - Add one row to the rules table.
   - Add Bad/Good examples in a dedicated section when the rule is non-trivial.

5. Test:
   - `pnpm exec eslint . --max-warnings=-1`
   - Create a temporary file with the bad pattern inside the project to verify it actually fires.

6. Commit (exactly one logical task per commit):
   - `chore: NEW eslint rule add your-rule-name` (rule + index.js + eslint.config.mjs + any code fixes to make it pass)
   - Separate commit if you touched only docs: `docs: update dev_readme-eslint.md for your-rule-name rule`
...
3. **Something inside `eslint-local-rules/*.js` uses a removed context API.** ESLint 9 removed
...
rules live in `eslint-local-rules/*.js`, wired up as `local-rules/<name>` in `eslint.config.mjs`.
...
| `local-rules/no-zustand-selector`                        | custom                                                                        | warn                  | `useStore(state => state.value)` selector-style usage - a hook imported from a path containing `store`/`zustand` (excluding zustand's own `create`/`createStore`) called with an arrow/function argument. Destructure the whole store instead: `const { value } = useStore()` in a component, `useStore.getState()` in a non-component file                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `local-rules/zustand-state-setter-pairing`               | custom                                                                        | warn                  | within a zustand store's returned object, a state property and its setter (any recognized verb - `set`/`add`/`reset`/`clear`/`toggle`/`upd`/`increase`/`decrease`/`remove`/`push`/`hadd`/`hdel`/`hupd`) aren't adjacent, or aren't followed by a blank line before the next property/pair. Skips a state name matched by more than one setter-shaped property (ambiguous - e.g. `ingredient` touched by `setIngredient`/`addIngredient`/`removeIngredient`) rather than guessing which one is "the" pair                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `local-rules/prefer-inline-zustand-setter`               | custom                                                                        | warn, **autofixable** | a zustand store method with a block body wrapping a single `return`/expression statement, where collapsing it to a one-line arrow function would still fit under 130 chars - flags the block-body version and autofixes it to `key: params => expr`, respecting this project's `arrowParens: "avoid"` (no parens around a single simple param) and stripping a trailing comma that's only valid before a multi-line closing bracket                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `local-rules/no-req-json-catch`                          | custom                                                                        | warn                  | `req.json().catch(...)` inside an API route - this swallows JSON parse failure and encourages noisy wrappers like `(await req.json().catch(() => ({})))`; await `req.json()` directly and use a normal surrounding `try/catch` only when invalid JSON is a real branch                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `local-rules/arrow-fn-only-for-hooks`                    | custom                                                                        | warn                  | a **module-top-level** `const x = () => {}` / `const x = function () {}` where `x` isn't a hook name (doesn't start with `use`) - components and plain helpers must use `function x() {}` instead. Arrow functions passed as call _arguments_ (`useCallback(() => {})`, `.map(item => ...)`, JSX inline handlers) are never flagged, and neither is a `const x = () => {}` declared _inside_ another function's body (e.g. `const listener = (event) => handlerRef.current(event)` inside a `useEffect`, kept as a named local so it can be passed to both `addEventListener` and `removeEventListener`)                                                                                                                                                                                                                                                                                                                                        |
| `local-rules/no-inner-component`                         | custom                                                                        | warn                  | defining a functional component (`const Foo = () => <div/>` or `function Foo() {}`) or render helper (`const renderXxx = ...` / `function renderXxx`) inside another component. Only names starting with capital letter or exactly `render` + capital are flagged. Normal handlers are allowed. Hoist to module scope. See "Component structure rules" section below for examples.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `local-rules/api-folder-requires-api-namespace`          | custom                                                                        | warn                  | a `.ts`/`.tsx` source file under `app/api/` (except `app/api/api.d.ts`) with no `API.` reference anywhere in the file - route files and colocated helpers under `app/api/**` must participate in the `API` namespace contract instead of staying as untyped internal helpers. In jompanion the contract file is `api.d.ts` at the repo root, not `app/api/api.d.ts` - see the "In this project" note under "SDK / API route / api.d.ts architecture" below                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `local-rules/api-type-req-resp-suffix`                   | custom                                                                        | warn                  | a type alias or interface in any `api.d.ts` file ending in `Request` or `Response` - API namespace contracts use `Req` and `Resp` suffixes instead, e.g. `MoveEmailToFolderReq` / `MoveEmailToFolderResp`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `local-rules/no-void-only-jsx-handler`                   | custom                                                                        | warn                  | a JSX event handler arrow whose whole block body is `void someCall(...)` - use a direct implicit-return wrapper like `event => fn(event)` instead                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `local-rules/arrow-parens-avoid`                         | custom                                                                        | warn, **autofixable** | a single simple identifier wrapped in parens as the only arrow parameter, violating this project's `arrowParens: "avoid"` style. Bad: `(application) => ...`. Good: `application => ...`. Exempt when the sole parameter is destructured, has defaults/rest, is inside a type annotation, or the parens contain a non-simple inner expression. `--fix` removes the outer parens when safe.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `local-rules/function-body-own-line`                     | custom                                                                        | warn                  | a function whose body does real multi-line work (more than one statement, or a single `return` of an object/array literal with more than one property/element) where the opening `{` isn't the last token on the signature line - e.g. `function extractUTMParams(params = {}) { return ({` crams `return (` onto the signature line. A trivial single-statement body is exempt regardless of formatting.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `local-rules/console-log-line-number`                    | custom                                                                        | warn, **autofixable** | this codebase's `console.log(<line-number>, ...)` debug convention - the numeric first-argument literal is meant to mark which line the call sits on, so a terminal log line can be traced back to its call site without a stack trace. Flags (and autofixes) the literal once it drifts out of sync with the call's real `node.loc.start.line`, which happens the moment code is inserted/removed above the call.                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `local-rules/no-redundant-return-parens`                 | custom                                                                        | warn, **autofixable** | redundant parens around a `return`ed object/array literal (`return ({ a: 1 })` -> `return { a: 1 }`) - the parens only disambiguate an arrow function's implicit-return body (`() => ({ a: 1 })`) from a block body, which isn't ambiguous after an explicit `return` keyword. Not flagged: `() => ({ a: 1 })` (arrow implicit return - parens are load-bearing there).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
### Arrow parameter parens style (`arrow-parens-avoid`)

This project uses `arrowParens: "avoid"`, meaning a single bare identifier parameter doesn't need parens.
Multi-parameter, destructured, default/rest, typed, or non-simple parameters still keep their parens
because they're load-bearing rather than stylistic.

**Bad** - parens around a single simple identifier:
```ts
const apps = (applications) => applications.filter((application) => application.status === "pending")
// named param:                                                ^^^^^^^^^^^^^^^^^^^^ redundant
```

**Good** - no parens around a single simple identifier:
```ts
const apps = (applications) => applications.filter(application => application.status === "pending")
// named param:                                                ^^^^^^^^^^^^^^ bare identifier is fine
```

**Still valid** - these keep their parens because the parameter shape requires them:
```ts
const f1 = (x, y) => x + y
const f2 = ({a}) => a
const f3 = (x = 1) => x
...
**Not flagged** - these are outside this rule's scope:
```ts
const f6 = () => ({a: 1})    // zero params, n/a
const f7 = () => 1            // zero params, n/a
const unit = () => ({a})      // object literal implicit return needs parens around the arrow body, not the param
```
...
| `max-len`                                                | ESLint core                                                                   | warn                  | a line over 130 characters - `code: 130`, with `ignoreUrls` only (`ignoreStrings`/`ignoreTemplateLiterals`/`ignoreRegExpLiterals`/`ignoreComments` are all left off on purpose, since the case this was added for is an unbreakable long regex literal that Prettier's own `printWidth: 120` can't wrap)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `import/newline-after-import`                            | `eslint-plugin-import` (already loaded transitively via `eslint-config-next`) | warn, **autofixable** | missing blank line between the last import and the next statement - `{ count: 1 }` requires exactly one                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
Return-shape checks attribute each `return {...}` to its nearest enclosing function and only count
returns belonging to a `use[A-Z]*`-named function - a module-scope helper in the same file (e.g. an
`insertDBFood` defined above the hook) whose `.map()`/`.filter()` callbacks return object literals
is not part of the hook's return shape and is ignored.
...
(`getSupabaseServer`/`getSupabaseRouteHandlerClient`/`createImageBitmap`/`json`/`getCookie`) - these
match the read-verb pattern by name shape (`get*`, or `json` which happens not to but was still
noisy) but aren't this codebase's SDK read convention: `getSupabaseServer`/
`getSupabaseRouteHandlerClient` return a client/factory, not data (jompanion has both - one for
server components, one for route handlers); `createImageBitmap`/`Response.json()` are built-in Web
APIs. If this rule fires on another such case, add the method name to `EXCLUDED_METHOD_NAMES` in
`eslint-local-rules/response-variable-naming.js` rather than suppressing it file-by-file.
...
#### No creating components or render helpers inside components (`no-inner-component`)

**Bad** (recreated every render, closes over local state):
```ts
export const ApplicationCard = memo(function ApplicationCard({ application }: ApplicationCardProps) {
  const { showTooltips } = useShowTooltipsStore()

  const renderButton = (onClick: () => void, label: string, borderClass: string, tooltipText: string) => {
    // ...
  }
...
**Good** - declare the helper at module scope above the component:
```ts
function renderButton(showTooltips: boolean, onClick: () => void, label: string, borderClass: string, tooltipText: string) {
  // implementation
}
...
export const ApplicationCard = memo(function ApplicationCard({ application }: ApplicationCardProps) {
  ...
  return (
    ...
    {renderButton(showTooltips, () => onAccept(application.id), "Accept", "border-success", "...")}
  )
})
```

#### Arrow functions only for hooks + no local functions in deps (`arrow-fn-only-for-hooks` + `no-function-in-deps`)

**Bad:**
```ts
const DbBackupModal = () => { ... }   // component as arrow
const someFn = () => { ... }          // plain helper as arrow at top level
useEffect(() => { ... }, [someLocalFn]) // local fn in deps
```
...
export const useKeyboardHoursNavigation = () => { ... }  // only hooks use arrows

const setEntityRedis = useCallback(async (entityRedisUrl: string) => { ... }, [])

// Never put local functions in deps, even useCallback ones
useEffect(() => {
  // use ref pattern instead of depending on the function
}, []) // no local fn here
```

useState setters (`set*`) and zustand actions are exempt.
...
`eslint-local-rules/imports-order.js`, not to silently drop the check.
...
### Zustand rules (`no-zustand-selector`, `zustand-state-setter-pairing`, `prefer-inline-zustand-setter`), in depth

`zustand-state-setter-pairing` and `prefer-inline-zustand-setter` both detect a zustand store by
finding the object literal passed to (or returned from a function passed to) `create(...)` -
shared via `eslint-local-rules/utils/findZustandStoreObjectLiterals.js` so both rules agree on what
counts as "the store."

**No selector style** - always destructure the whole store, never select a slice with a callback:

```ts
// wrong
const food = useCartStore(state => state.food)

// correct - in a .tsx component
const { food } = useCartStore()

// correct - in a .ts (non-component) file
const { food } = useCartStore.getState()
```

**State + setter pairing** - a state property must sit directly next to its own setter (any
recognized verb, not just `set`), with a blank line after each pair:
...
When a state name is matched by more than one setter-shaped property (e.g. `ingredient` touched by
`setIngredient`, `addIngredient`, AND `removeIngredient` in `useAddNewIngredient.ts`), the rule
skips pairing entirely for that name rather than guessing which one is "the" pair - flagging all
three for adjacency to `ingredient` would be wrong, since they're independent action methods that
happen to touch the same state, not a single state+setter relationship.

**Prefer inline setters** - collapse a block-bodied method to a one-line arrow function once it
fits under 130 chars:
...
// correct (95 chars, fits)
setIngredient: data => set(state => ({ ingredient: { ...state.ingredient, ...data } })),
```

The autofix respects this project's `arrowParens: "avoid"` (a single simple param never gets
wrapped in parens - `data =>`, not `(data) =>`) and strips a trailing comma that was only valid
before a multi-line closing bracket, so collapsing `{ ingredient: undefined, }` doesn't leave a
dangling comma before the `}` on one line. Verified on `app/(admin)/store/useAddNewIngredient.ts`
(`setIngredient`, `removeIngredient`) with a `tsc --noEmit` check after fixing to confirm no type
errors were introduced.

### Some existing files intentionally left non-compliant

The paragraph below describes a real inconsistency found in the original project this rule set was
authored for (9 `useXxxStore.ts` files not calling `persist`). In **this** project (jompanion),
`hook-naming-convention` has zero violations as of the last full lint pass - there's no `useXxxStore.ts`
file with a mismatched shape here. Left as reference for what this class of finding looks like:
...
8. Each distinct realtime concern (typing indicator, ticket updates, message updates, etc.) must
   ...
   component `useEffect`.
9. When a `className`/`style` JSX string attribute is too long for one line and gets manually
   wrapped across multiple lines, indent the continuation lines to align under the first character
   after the opening quote (not just one flat indent step deeper):
   ```tsx
   <main
   ...
   ```
   Not ESLint-enforced: the target column depends on everything to the left of the string on that
   line (JSX nesting depth, attribute name length, tag name) - it's fine to check mechanically in
   principle, but re-fires on every unrelated edit that shifts that column (indenting the block one
   level, renaming the attribute), so it's judgment-call/code-review only rather than a hard rule.
10. Prefer `null` over empty strings where the absence of a value matters - e.g. keep a field
...
11. Never fetch data if you can get it from state (exception: something massive, e.g. email
    `text_html`).
...
4. Request bodies should use `satisfies API.XReq`.
...
**In this project (jompanion)**: the per-entity `app/ts/namespaces/api/<entity>/api.d.ts` layout
above is the general pattern from the project this rule set was authored for - jompanion instead has
a single root-level `api.d.ts` (declaring `namespace API` globally), extending the `RateLimit*` types
that already existed there before this pass with `AuthLogin*`, `AuthRegister*`, `CategoryCreate*`,
`CompanionCreate*`/`CompanionUpdate*`, `ChatGet*`/`ChatPost*`, `TurnstileVerify*`, `StripeInfo*`, and
`WebhookSubscription*`. There's also no per-entity SDK-class layer yet (`RateLimitSDK` is the only
one) - routes are typed directly with `satisfies API.XReq`/`API.XResp` at the fetch call site and in
the route handler, without an intermediate SDK class. `api-folder-requires-api-namespace` only checks
that a file under `app/api/` references `API.*` somewhere, so it's satisfied either way; adopt the
full per-entity SDK-class pattern above if this project's API surface grows enough to want it.
...
export class ProductsSDK {
  async selectDBProducts(start: number, end: number) {
    ...
    const responseJson: API.ProductsSelectResp = await response.json()
    if ("error" in responseJson) return responseJson.error
    else return responseJson
  }

  async addProduct(request: API.ProductsAddReq) {
    const response = await fetch("/api/products/add", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(request satisfies API.ProductsAddReq),
      cache: "no-store",
    })

    const responseJson: API.ProductsAddResp = await response.json()
    if ("error" in responseJson) return responseJson.error
    else return responseJson
  }
}
...
export async function POST(req: Request) {
  const body = (await req.json()) as API.ProductsSelectReq

  if (!body.ownerId || typeof body.ownerId !== "string") {
    return NextResponse.json({ error: `ownerId missing: ${body.ownerId}` } satisfies API.ProductsSelectResp, {
      status: 400,
    })
  }
  ...
  if (typeof response === "string") {
    return NextResponse.json({ error: response } satisfies API.ProductsSelectResp, { status: 500 })
  }

  return NextResponse.json(response satisfies API.ProductsSelectResp, { status: 200 })
}
```

```ts
// ── api/ai-decisions/manage-emails/reply/select-decisions  ──────────────────────────────────────
type SelectReplySettingsReq = { encryptedEnvsClient: string[] }
type SelectReplySettingsResp = { settings: import("@/ts/TAIEmailDecisions").TAIReplyDecisionsSettingDB[] } | { error: string }

// ── api/ai-decisions/manage-emails/reply/regenerate  ─────────────────────────────────────────────
type RegenerateReplyDecisionReq = {
...
type RegenerateReplyDecisionResp = { suggestion: import("@/ts/TAIEmailDecisions").TReplySuggestion } | { error: string }
```
