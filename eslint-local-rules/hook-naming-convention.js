...
// Collects properties of `return {...}` statements that belong to the file's use* hook itself.
// Each return is attributed to its nearest enclosing function, and only functions named use[A-Z]*
// count - so a module-scope helper in the same file (e.g. an insertDBFood above the hook) whose
// .map()/.filter() callbacks return object literals no longer pollutes the hook's return shape.
function getHookReturnProperties(programNode, scopeManager) {
  const returnProperties = []

  function walk(node, nearestFnName, nameForNextFn) {
    ...
    // Track which function a child return belongs to: a FunctionDeclaration carries its own name,
    // an arrow/function expression takes the name of the VariableDeclarator it was assigned to.
    let childFnName = nearestFnName
    let childNameForNextFn = null
    if (node.type === "FunctionDeclaration") childFnName = node.id ? node.id.name : null
    else if (node.type === "ArrowFunctionExpression" || node.type === "FunctionExpression") childFnName = nameForNextFn
    else if (node.type === "VariableDeclarator" && node.id.type === "Identifier") childNameForNextFn = node.id.name

    for (const key in node) {
      if (key === "parent") continue
      const value = node[key]
      if (Array.isArray(value)) value.forEach(child => walk(child, childFnName, childNameForNextFn))
      else if (value && typeof value.type === "string") walk(value, childFnName, childNameForNextFn)
    }
  }

  walk(programNode, null, null)
      ...
      return {
        Program(node) {
          ...
          if (isHandlersFile || isAutoUpdateFile) {
            const returnProperties = getHookReturnProperties(node, sourceCode.scopeManager)
