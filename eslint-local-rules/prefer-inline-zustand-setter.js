...
// Flattening whitespace to one line is only safe when there's no nested block with more than one
// statement inside the expression being collapsed - e.g. set(state => { const x = ...; return y })
// has two statements in its own block body, and collapsing away the newline between them would
// merge "...state.uploads" and "return {...}" onto one line with no separator, corrupting the
// syntax (ASI doesn't save this - it only inserts a semicolon at specific token boundaries, not
// wherever a newline used to be). A block with zero or one statements has no such boundary to lose.
function hasMultiStatementBlock(node, seen) {
  if (!node || typeof node !== "object" || seen.has(node)) return false
  seen.add(node)
  if (node.type === "BlockStatement" && Array.isArray(node.body) && node.body.length > 1) return true
  for (const key of Object.keys(node)) {
    if (key === "parent" || key === "loc" || key === "range") continue
    const value = node[key]
    if (Array.isArray(value)) {
      for (const item of value) {
        if (item && typeof item === "object" && typeof item.type === "string" && hasMultiStatementBlock(item, seen)) return true
      }
    } else if (value && typeof value === "object" && typeof value.type === "string") {
      if (hasMultiStatementBlock(value, seen)) return true
    }
  }
  return false
}
  ...
  "prefer-inline-zustand-setter": {
    ...
    create(context) {
      ...
      function checkProperty(property) {
        ...
        if (hasMultiStatementBlock(expression, new Set())) return

        // A comment anywhere inside the block (e.g. a trailing "// why" after the set(...) call)
        // has nowhere safe to go once the body collapses to one line - replacing the property's
        // whole range would silently delete it, which breaks the "keep existing comments" rule.
        if (sourceCode.getCommentsInside(blockBody).length > 0) return
